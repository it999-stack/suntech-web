...
import { useConfirmPileImport, usePreviewPileImport } from '../hooks/usePiles'
import type { PileImportPreview } from '../types/piles.types'

interface PileImportDialogProps {
  siteId: string
  open: boolean
...
export function PileImportDialog({ siteId, open, onOpenChange }: PileImportDialogProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null)
  const [preview, setPreview] = useState<PileImportPreview | null>(null)
  const [isDownloadingTemplate, setIsDownloadingTemplate] = useState(false)
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(20)
  const fileInputRef = useRef<HTMLInputElement>(null)

  const previewImport = usePreviewPileImport()
  const confirmImport = useConfirmPileImport()
  ...
  async function handleDownloadTemplate() {
    ...
    try {
      await pilesService.downloadPileImportTemplate(siteId)
    } catch (error) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      toast.success(`${result.created} pile${result.created === 1 ? '' : 's'} imported`)
      if (result.failedRows.length > 0) {
        toast.error(`${result.failedRows.length} row(s) could not be imported — data may have changed`)
      }
      handleOpenChange(false)
    } catch (error) {
      toast.error(getErrorMessage(error, 'Failed to import piles'))
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="flex max-h-[85vh] w-full flex-col gap-4 sm:max-w-[960px]">
        <DialogHeader>
          <DialogTitle>Import Piles</DialogTitle>
        </DialogHeader>

        {!preview ? (
          <div className="flex flex-col gap-4">
            <p className="text-sm text-muted-foreground">
              Upload an Excel sheet with one row per pile. Each row's Location and Dimension must already
              exist for this site — download the template below to see the exact columns and this site's
              current Locations and Dimensions.
            </p>
                  ...
                  {pagedRows.map((row) => (
                    <TableRow key={row.rowNumber}>
                      ...
                      <TableCell className="text-muted-foreground">
                        {row.dimensionDia != null && row.dimensionDepth != null
                          ? `${row.dimensionDia}/${row.dimensionDepth}`
                          : '—'}
                      </TableCell>
        ...
        <DialogFooter>
          ...
          {!preview ? (
            ...
            <Button onClick={handleConfirm} loading={confirmImport.isPending} disabled={validRows.length === 0}>
              Import {validRows.length} pile{validRows.length === 1 ? '' : 's'}
            </Button>
          )}
        </DialogFooter>
      </DialogContent>
